...
import { NutritionCacheTable } from "./nutrition-cache-table"
...
export function UserSettingsModal({ open, onClose }: Props) {
  const { targets, saveTargets, nutritionCacheItems, loading, fetchNutritionCache, deleteNutritionCacheItem } = useUserSettings()
  ...
  useEffect(() => {
    if (open) {
      setForm({ ...targets })
      setError(null)
      // Reload cache from backend every time settings are opened
      fetchNutritionCache()
    }
  }, [open, targets, fetchNutritionCache])
  ...
  const handleDeleteCacheItem = async (id: string) => {
    try {
      await deleteNutritionCacheItem(id)
      // Refresh the list after deletion
      await fetchNutritionCache()
    } catch (e: any) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white w-full max-w-3xl rounded-lg shadow-lg p-6">
        ...
        <div className="mb-8">
          <h3 className="text-md font-semibold mb-4 text-gray-900">Daily Nutrition Targets</h3>
        ...
        <div className="mt-8">
          <NutritionCacheTable 
            items={nutritionCacheItems} 
            onDelete={handleDeleteCacheItem} 
          />
        </div>
      </div>
    </div>
